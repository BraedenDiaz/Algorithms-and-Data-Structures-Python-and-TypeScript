...
import { Vertex } from "./GenericVertex";
import { heapValueTypes, MinHeap } from "./MinHeap";

/**
 * This is a generic graph that allows any type the user specifies to be used with the
 * vertices.
 * 
 * Note that this class has some Algorithms and Data Structures that are shown in later
 * sections. Please refer to those sections if you need more help understanding some of the
        ...
        while (minHeap.count > 0 && (distancesAndPaths.get(targetVertex.value)![0] === Infinity))
        {
            ...
            for (let [neighbor, edgeWeight] of this._vertices.get(currentVertexValue)!.neighbors)
            {
                ...
                if (newDistance < distancesAndPaths.get(neighbor)![0])
                {
                    ...
                    count++;
                    console.log("\nAt");
                    console.log(currentVertexValue);
                    console.log();
                }
            }
        }

        console.log(`Found a path from ${startVertex.value} to ${targetVertex.value} in ${count} steps.`);
