...
def copy_binary_search(sorted_list, target):
  if not sorted_list:
    return 'value not found'
  ...
  if mid_val > target:
    ...
    return copy_binary_search(left_half, target)
  
  if mid_val < target:
    right_half = sorted_list[mid_idx + 1:]
    result = copy_binary_search(right_half, target)
...
print(copy_binary_search(sorted_values, 16))
